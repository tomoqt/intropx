...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            if idx.size(1) <= self.config.block_size:
                idx_cond = idx
                dist_cond = distributions
            else:
                idx_cond = idx[:, -self.config.block_size:]
                dist_cond = distributions[:, -self.config.block_size:, :]
            ...
            probs = probs.unsqueeze(1)  # Shape: (b, 1, vocab_size)
            distributions = torch.cat((distributions, probs), dim=1)
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        print(f"using fused AdamW: {use_fused} with intropix")
